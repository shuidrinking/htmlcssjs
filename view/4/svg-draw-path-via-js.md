...
<script type="text/javascript">
	function drawAllSvgPath(){
		const paths = document.querySelectorAll('.draw-path');
		paths.forEach((p)=>{
			const l = p.getTotalLength() + 1; //将要绘制的路径起点的偏移量设置为svg的长度
			p.style.setProperty('--len', l);
		});
	}
	setInterval(drawAllSvgPath, 100);
</script>

<div class="demobox">
<div class="button lightBlueButton" onclick="drawAllSvgPath()">完成剩余部分的绘制</div>
<svg>
	<line class="draw-path" x1="10" y1="100" x2="390" y2="100" stroke-width="2"/>  
</svg>
<svg>
	<circle class="draw-path" cx="50%" cy="50%" r="40" stroke="black" fill="none" stroke-width="3"></circle>
</svg>
<svg class="icon"
